		...
		for (int i = 0; i < maxTryTimes; i++) {
			...
			try {
				...
				if (imgnameSelector == null || idSelector == null || timeLongSelector == null)
					throw new RuntimeException(TvSiteEnum.getEnumByUrl(url).getUrl() + ",url=" + url + "目前不支持获取图片列表");
				if (imgnameSelector != null || idSelector != null || timeLongSelector != null) {
					...
					Elements nextPageElement = (nextPageSelector.contains(".empty") ? idElement
							: doc.select(nextPageSelector));
					String nextUrl = "";
					...
					if (TvSiteEnum.getEnumUrl(url) == TvSiteEnum.YinYueTai.getUrl()) {
						for (Element elss : imgNameElement) {
							System.out.println(
									"正在获取网站：【" + url + "】的第【" + (a + 1) + "/" + imgNameElement.size() + "】组的视频信息。");
							...
							if (DbUtil.getresultOftvUrl(idElement.get(a).attr("data-video-id"))) {
								...
								tvModel[a].setDateLong(
										timeLongElement.size() <= a ? "未知" : timeLongElement.get(a).text());
								...
								System.out.println("已有重复数据：name:【" + elss.attr("title") + "】url：【"
										+ idElement.get(a).attr("data-video-id") + "】");
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							}
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(idElement.get(a).attr("href"))) {
								...
								tvModel[a].setContext(elss.attr("title") + "--"
										+ (starsElement.size() <= a ? "未知" : starsElement.get(a).text()));
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							} else {
								System.out.println("已有重复数据：name:【" + elss.attr("title") + "】url：【"
										+ idElement.get(a).attr("href") + "】");
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							}
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(elss.absUrl("href"))) {
								...
								System.out.println(
										"已有重复数据：name:【" + elss.attr("title") + "】url：【" + elss.absUrl("href") + "】");
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							}
						}
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(elss.absUrl("href"))) {
								...
								tvModel[a].setCrawlTime(StringUtil.getNowTime("yyyy-MM-dd HH:mm:ss"));
								tvModel[a].setTvUrl(elss.absUrl("href"));
								tvModel[a].setContext(elss.text() + "--"
										+ (starsElement.size() <= a ? "未知" : starsElement.get(a).text()));
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							} else {
								System.out
										.println("已有重复数据：name:【" + elss.text() + "】url：【" + elss.absUrl("href") + "】");
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							}
						}
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(idElement.get(a).absUrl("href"))) {
								...
								tvModel[a].setContext(elss.attr("alt") + "--"
										+ (starsElement.size() <= a ? "未知" : starsElement.get(a).text()));
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							} else {
								System.out.println("已有重复数据：name:【" + elss.attr("alt") + "】url：【"
										+ idElement.get(a).absUrl("href") + "】");
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							}
						}
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(elss.absUrl("href"))) {
								...
								tvModel[a].setCrawlTime(StringUtil.getNowTime("yyyy-MM-dd HH:mm:ss"));
								tvModel[a].setTvUrl(elss.absUrl("href"));
								tvModel[a].setContext(idElement.get(a).text() + "--"
										+ (starsElement.size() <= a ? "未知" : starsElement.get(a).text()));
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							} else {
								System.out.println("已有重复数据：name:【" + idElement.get(a).text() + "】url：【"
										+ elss.absUrl("href") + "】");
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							}
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(idElement.get(a).absUrl("href"))) {
								...
								tvModel[a].setCrawlTime(StringUtil.getNowTime("yyyy-MM-dd HH:mm:ss"));
								tvModel[a].setTvUrl(idElement.get(a).absUrl("href"));
								tvModel[a].setContext(elss.attr("alt") + "已播放："
										+ (starsElement.size() <= a ? "未知" : starsElement.get(a).text()));
								tvModel[a].setNetxUrl(nextUrl);
								a++;
							} else {
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(idElement.get(a).absUrl("href"))) {
								...
								tvModel[a].setTvUrl(idElement.get(a).absUrl("href"));
								tvModel[a].setContext(elss.attr("title") + "--"
										+ resu.substring(resu.indexOf("作者:"), resu.indexOf("查看:")));
								tvModel[a].setNetxUrl(nextUrl);
						...
						for (Element elss : imgNameElement) {
							...
							if (DbUtil.getresultOftvUrl(idElement.get(a).absUrl("href"))) {
								...
								tvModel[a].setPlatformName(contexts.get("title"));
								tvModel[a].setAddTime(StringUtil.getNowTime("yyyy-MM-dd HH:mm:ss"));
								tvModel[a].setDateLong(timeLongElement.get(a + (a - 1) + 1).text() + "--"
										+ timeLongElement.get(a + (a + 1)).text());
								tvModel[a].setCrawlTime(StringUtil.getNowTime("yyyy-MM-dd HH:mm:ss"));
								tvModel[a].setTvUrl(idElement.get(a).absUrl("href"));
